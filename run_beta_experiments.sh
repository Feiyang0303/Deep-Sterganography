#!/bin/bash
#SBATCH --job-name=beta_exp
#SBATCH --partition=ALL
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=2-00:00:00
#SBATCH --output=output/beta_exp-%j.out
#SBATCH --error=output/beta_exp-%j-err.out

# Activate conda environment
source activate deep_steg

# Set LD_LIBRARY_PATH for GPU support
export LD_LIBRARY_PATH=$(python -c "
import nvidia, os
base = os.path.join(os.path.dirname(nvidia.__path__[0]), 'nvidia')
dirs = ['cudnn','cuda_runtime','cublas','cufft','cuda_cupti','cusolver','cusparse','nccl','nvtx']
print(':'.join(os.path.join(base, d, 'lib') for d in dirs))
")

mkdir -p output

echo "========================================="
echo "Training with beta=0.75"
echo "========================================="
python -u train.py \
    --epochs 1000 \
    --batch_size 32 \
    --beta 0.75 \
    --data_dir ./tiny-imagenet-200 \
    --output_dir ./output

echo "========================================="
echo "Training with beta=1.25"
echo "========================================="
python -u train.py \
    --epochs 1000 \
    --batch_size 32 \
    --beta 1.25 \
    --data_dir ./tiny-imagenet-200 \
    --output_dir ./output

echo "========================================="
echo "All beta experiments complete!"
echo "========================================="
